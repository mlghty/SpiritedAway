...
class SpiritedAway(QWidget):
    def __init__(self):    
        ...
        # Autostart checkbox
        self.autostart_checkbox = QCheckBox("Start with macOS")
    ...
    def check_for_user_activity(self):
        """Monitor user activity and minimize/restore windows accordingly"""
        while self.START:
            try:
                ...
                # Check for user input using macOS APIs
                # Get time since last input event
                last_event_time = CGEventSourceSecondsSinceLastEventType(
                    kCGEventSourceStateHIDSystemState,
                    kCGAnyInputEventType
                )
                
                if last_event_time < 0.1:  # If there was input in the last 0.1 seconds
                    self.last_input_time = current_time
                
                # Check mouse position
                current_cursor_pos = NSEvent.mouseLocation()
                if (current_cursor_pos.x != self.last_cursor_pos.x or 
                    current_cursor_pos.y != self.last_cursor_pos.y):
                    self.last_input_time = current_time
    ...
    def toggle_autostart(self, state):
        """Enable or disable autostart with macOS"""
        # macOS uses LaunchAgents for autostart
        launch_agent_dir = os.path.expanduser("~/Library/LaunchAgents")
        plist_file = os.path.join(launch_agent_dir, "com.spiritedaway.app.plist")
...
"""
                ...
                self.tray_icon.showMessage(
                    self.app_name,
                    "🚀 Autostart Enabled\nI'll start automatically with macOS!",
                    self.icon,
                    2000
                )
            else:
